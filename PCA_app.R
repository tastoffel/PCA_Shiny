  ...
  output$pca_intro <- renderUI({
    HTML(
      paste(
        ...
        tags$p("The first step to use this app, regarding the appliance of a PCA to your dataset, you should ensure that the file you're uploading is in the", tags$b(".csv"), "format!"),
        ...
        tags$p("- On the ", tags$b("Variance Explained")," tab you'll find a barplot representing the variane explained by each principal component, as well as a table with the eigenvalues, variance explained and the cumulative variance explained, for each PC."),
        ...
        tags$p("- On the ", tags$b("Biplot")," tab you'll find a 2-dimensional representation of the observations and the variables retained by the 2 PC's."),
        ...
        tags$h4("Have fun!"),
        tags$br(),
        tags$br(tags$b("Important note:")),
        tags$p("You should be aware that some information might be sensitive to upload, hence we recomend you to attribute some ficional values/names to sensitive variables such as Names, Salaries, Adresses, acording to", tags$b("Privacy Policy."))
        )
    )
  })
...
# Check if there are missing values and the user didn't select "Remove NA rows", retrieving an Error message
...
# Variance Explained Tab
  output$pc_criteria <- renderUI({
    HTML(
      paste(
      tags$p("The user should use the information provided below in order to decide how many principal components to retain according to following criteria:"),
      tags$p("- Total variance explained;"),
      tags$p("- Elbow point in the scree plot;"),
      tags$p("- Kaiser's criteria.")
      )
    )
  })
...
# Principal Component Structure Tab
  output$pc_instructions <- renderUI({
    ...
    HTML(
      ...
      tags$br(),
      tags$p("Select how many principal components to retain according to the criteria performed before. You can retain as many as the total number of original variables. Generally, you would select the number of components that explain a significant portion of the variance (e.g., > 70%).")
      )
    )  
  })
...
# Number of PC to retain, where the max choices are delimited to the variables selected
